...
Để biến điều này thành một cấu trúc dữ liệu an toàn cho luồng, chúng ta cần xác định các _đoạn mã quan trọng_ của mã của chúng ta, tức là phần (các) phần nào của mã chỉ có thể có một luồng tại một thời điểm. Trong ví dụ trên, các hàm `push`, `pop` và `is_empty` truy cập cùng một biến (tức là bộ nhớ) và tất cả các đoạn mã quan trọng cho ngăn xếp.
...
```
Mã trên ('phiên bản 2') chứa ít nhất một lỗi. Hãy dành một chút thời gian để xem bạn có thể tìm ra lỗi (các lỗi) và tìm ra hậu quả (các hậu quả).

Nếu ba luồng gọi `push()` cùng một lúc, khóa `m1` đảm bảo rằng chỉ có một luồng tại một thời điểm thao tác ngăn xếp (hai luồng sẽ cần đợi cho đến khi luồng đầu tiên hoàn thành (gọi unlock), sau đó luồng thứ hai sẽ được phép tiếp tục vào phần quan trọng và cuối cùng luồng thứ ba sẽ được phép tiếp tục sau khi luồng thứ hai đã hoàn thành).
...
```
Phiên bản 3 an toàn cho luồng (chúng tôi đã đảm bảo loại trừ lẫn nhau cho tất cả các phần quan trọng) tuy nhiên có hai điểm cần lưu ý:
...
void init() {
  sem_init(&sitems, 0, 0);
  sem_init(&sremains, 0, 10); // 10 khoảng trống
}
